      ...
      try {
        if (currentTable === 'players') {
          ...
          batch.set(playerRef, {
            name,
            phone,
            annual: annualAmount || 0,
            monthly: monthlyAmount || 0,
            pitch: pitchAmount || 0,
            matchDay: matchDayAmount || null,
          ...
          if (paymentsBatch.length >= MAX_BATCH) {
            const currentBatch = db.batch();
            paymentsBatch.forEach(payment => {
              const ref = db.collection('payments').doc();
              currentBatch.set(ref, payment);
            });
          ...
          if (expensesBatch.length >= MAX_BATCH) {
            const currentBatch = db.batch();
            expensesBatch.forEach(expense => {
              const ref = db.collection('expenses').doc();
              currentBatch.set(ref, expense);
            });
            await currentBatch.commit();
